...
export default function CreatePropertyScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (isNaN(priceNum) || priceNum < MIN_PRICE)
      return Alert.alert("Validation", "Price must be greater than EGP0.");
    if (priceNum > MAX_PRICE)
      return Alert.alert(
        "Validation",
        `Price cannot exceed EGP${MAX_PRICE.toLocaleString("en-IN")}.`,
      );
    ...
    >
      ...
      >
        ...
        >
          ...
          {/* Price */}
          <View className={sectionClass}>
            <Text className={labelClass}>Price (EGP)</Text>
            <TextInput
              className={inputClass}
            ...
            <Text className="text-xs text-gray-400 mt-1.5 ml-1">
              Valid range: EGP1 – EGP{MAX_PRICE.toLocaleString("en-IN")}
            </Text>
          </View>
